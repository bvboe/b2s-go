...
show_summary() {
    ...
    if check_gh_cli; then
        ...
        echo "  helm install bjorn2scan ./k8s-scan-server/helm/bjorn2scan \\"
        echo "    --set image.repository=ghcr.io/$(gh repo view --json nameWithOwner -q .nameWithOwner | tr '[:upper:]' '[:lower:]')/k8s-scan-server \\"
