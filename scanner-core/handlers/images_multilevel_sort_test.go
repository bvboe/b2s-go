...
func TestImagesMultiLevelSorting(t *testing.T) {
	tests := []struct {
		...
		{
			name:            "Sort by instance_count: sort_order, instance_count, image",
			sortBy:          "instance_count",
			sortOrder:       "DESC",
			expectedOrderBy: "ORDER BY status.sort_order ASC, instance_count DESC, image ASC",
		},
		{
...
func TestPodsMultiLevelSorting(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			sortBy:          "",
			sortOrder:       "ASC",
			expectedOrderBy: "ORDER BY status.sort_order ASC, instances.namespace ASC, instances.pod ASC, instances.container ASC",
		},
		{
			name:            "Sort by critical_count: includes all tie-breakers",
			sortBy:          "critical_count",
			sortOrder:       "DESC",
			expectedOrderBy: "ORDER BY status.sort_order ASC, critical_count DESC, instances.namespace ASC, instances.pod ASC, instances.container ASC",
		},
		{
			name:             "Sort by namespace: no duplicate namespace",
			sortBy:           "namespace",
			sortOrder:        "DESC",
			expectedOrderBy:  "ORDER BY status.sort_order ASC, namespace DESC, instances.pod ASC, instances.container ASC",
		...
		},
		{
			...
			sortBy:           "pod",
			sortOrder:        "ASC",
			expectedOrderBy:  "ORDER BY status.sort_order ASC, pod ASC, instances.namespace ASC, instances.container ASC",
			shouldNotContain: "pod ASC, instances.pod ASC",
		},
		{
			name:             "Sort by container: no duplicate container",
			sortBy:           "container",
			sortOrder:        "DESC",
			expectedOrderBy:  "ORDER BY status.sort_order ASC, container DESC, instances.namespace ASC, instances.pod ASC",
			shouldNotContain: "container DESC, instances.container ASC",
		},
	}
